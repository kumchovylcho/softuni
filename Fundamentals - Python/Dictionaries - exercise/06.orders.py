...
products = input()
while products != "buy":
    product_name, product_price, product_quantity = [float(item) if item[-1].isdigit() else item for item in products.split()]

    storage[product_name] = storage.get(product_name, {"price": 0, "quantity": 0})
